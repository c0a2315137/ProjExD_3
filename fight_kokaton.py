...
class Score:
    ...
    def update(self, screen: pg.Surface):
        img = self.fonto.render("スコア:" + str(self.score), 0, self.color)
        img_rct = img.get_rect()
        img_rct.center = self.center
        screen.blit(img, img_rct)


class Explosion:
    def __init__(self):
        img_ex1 = pg.image.load("fig/explosion.gif")
        img_ex2 = pg.transform.flip(img_ex1, True, True)
        self.imgs = [img_ex1, img_ex2]
        self.center = rct.bombs[i]
        self.life = 10
    def update(self, screen: pg.Surface):
        self.life -= 1
        if life > 0:
